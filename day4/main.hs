...
allValidators = [byr, iyr, eyr, hgt, hcl, ecl, pid] where
...
validField field val = any (f field val) $ zip allFields allValidators where
  f field val (fieldf, valf) = (field == "cid") || ((field == fieldf) && valf val)

toPassports :: String -> [String]
toPassports str = map unwords $ splitOn [""] $ lines str

validPassport1 :: String -> Bool
validPassport1 str = all (`elem` fields) allFields where
  entries = splitOn " " str
  splitEntries = map (splitOn ":") entries
  fields = map head splitEntries

validPassport2 :: String -> Bool
validPassport2 str = all (`elem` fields) allFields && and (zipWith validField fields vals)  where
  entries = splitOn " " str
  splitEntries = map (splitOn ":") entries
  fields = map head splitEntries
